...
import { routinesStore } from "../firebase/RoutinesStore";
import { Routine, RoutineItem } from "../firebase/routinesService";
...
export const RoutinesManager = observer(() => {
  ...
  }, [routinesStore.routines, draftItems]);
...
function Header() {
  return (
    <header className="flex items-center gap-3">
      <div className="text-2xl font-bold">Gim • Rutinas • {SessionManager.instance.user.displayName} {SessionManager.instance.user.uid}</div>
    </header>
  );
}
...
const UseRoutineModal = observer(({
  routine,
  onCancel,
  onSave,
}: {
            ...
            <div key={it.id} className="rounded border border-black/10 dark:border-white/20 p-3">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 items-center">
                ...
                <div className="text-right text-sm">
                  {RoutinesStore.instance.lastWeights[it.exercise] ?? "—"}
                </div>
